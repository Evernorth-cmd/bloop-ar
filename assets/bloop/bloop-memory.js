...
async function match(video){const items=await all();if(!items.length)return null;const q=await embed(video);let best=null;for(const item of items){for(const v of item.vectors||[]){const score=cos(q,v);if(!best||score>best.score)best={id:item.id,name:item.name,score}}}return best&&best.score>=0.93?best:null}
